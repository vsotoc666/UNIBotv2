...
def process_pdf_rag(documento):
    ...
    try:
        ...
        if failed_chunks == len(chunks):
            print(f"[RAG] ❌ '{documento.nombre}' falló: ningún fragmento pudo indexarse.")
            return
        documento.procesado = True
        documento.save(update_fields=['procesado'])
        if failed_chunks > 0:
            print(f"[RAG] ⚠️ '{documento.nombre}' procesado parcialmente ({len(chunks) - failed_chunks}/{len(chunks)} fragmentos).")
        else:
