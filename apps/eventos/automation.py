...
def ejecutar_flujo_post_inscripcion(inscripcion: Inscripcion) -> dict:
    if _inscripcion_cancelada(inscripcion):
        return {"mail_ok": False, "clipup_ok": False, "notion_url": "", "prioridad": ""}
...
def enviar_certificado_si_asistio(inscripcion: Inscripcion) -> bool:
    ...
    # Enviar primero: si falla, no se sube el archivo al storage.
    ...
    # Solo guardar en storage después de confirmar envío exitoso.
    ...
    # Actualizar datos_extra con la URL del certificado sin crear una nueva página en Notion
    # (la página de Notion ya fue creada en ejecutar_flujo_post_inscripcion).
    extras = dict(inscripcion.datos_extra or {})
    try:
        extras["certificado_url"] = inscripcion.certificado_archivo.url
    except Exception:
        pass
    inscripcion.datos_extra = extras
    inscripcion.save(update_fields=["certificado_archivo", "certificado_enviado_en", "datos_extra"])
    return True
...
def cerrar_evento_y_generar_acta(evento: Evento, comentarios: str = "") -> dict:
    if evento.cerrado:
        return {
            ...
            "tareas_pendientes": [],
            "acta_notion_url": evento.acta_notion_url,
            "ya_cerrado": True,
        }
...
def ejecutar_recordatorios_eventos(agrupacion_id: Optional[int] = None) -> dict:
    ...
    for evento in events_qs:
        ...
        for ins in inscritos:
            ...
            if delta <= timedelta(hours=24, minutes=30) and delta > timedelta(hours=2, minutes=20):
