...
def sync_inscritos(request, evento_id: int):
    ...
    if evento.tipo_inscripcion != "google_form":
        return {"status": "error", "message": "Este evento usa formulario interno."}
    if not (evento.form_externo_url or "").strip():
        return {"status": "error", "message": "Falta el link del Google Form."}
