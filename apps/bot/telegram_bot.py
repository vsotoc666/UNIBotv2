...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not args:
        await update.message.reply_text("👋 ¡Hola! Soy UNIBot.\nPara usarme, necesitas hacer clic en el enlace de Telegram que aparece en el Dashboard web de tu agrupación.")
        return
    ...
    if nombre_agrupacion:
        await update.message.reply_text(f"✅ ¡Enlazado exitosamente a la agrupación: *{nombre_agrupacion}*!\n\nPuedes hacerme preguntas sobre eventos o reglamentos y te responderé.")
    else:
...
async def responder_mensaje(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lista_eventos = []
    for e in eventos:
        slug = await sync_to_async(getattr)(e.formularioregistro, 'slug')
        lista_eventos.append(f"- {e.titulo} (Inicia: {e.fecha_inicio.strftime('%Y-%m-%d %H:%M')}). Registro: http://127.0.0.1:8000/evento/registro/{slug}/")
    
    eventos_str = "\n".join(lista_eventos) if lista_eventos else "No hay eventos próximos programados."

    # 3. Prompt Rígido e Inyectado (Gemini 2.5 Flash)
    prompt = f"""
    ...
    CONOCIMIENTO OFICIAL DE LA AGRUPACIÓN:
    ---
    {config.texto_extraido}
    ---
...
def main():
    ...
    app.add_handler(CommandHandler("start", start_command)) # <--- Nuevo Handler de Deep Linking
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, responder_mensaje))
    
    print("🤖 UNIBot (Multi-Tenant) iniciado. Presiona Ctrl+C para detener.")
    app.run_polling()